...
public class DefaultJSONParser extends AbstractJSONParser implements Closeable {
        ...
        try {
            ...
            for (;;) {
                ...
                if (ch == '"') {
                    ...
                    throw new JSONException("syntax error");//return null;//changed!
                } else if (ch == ',') {
                    throw new JSONException("syntax error");
                ...
                if (ch == '"') {
                    ...
                    throw new JSONException("syntax error, position at " + lexer.pos() + ", name " + key);//return null; //changed
                    }
                }
    ...
    /**
     * start() and end() are added by Zhenbang
     */
    ...
    public Object parse(Object fieldName) {
        ...
        switch (lexer.token()) {
            ...
            case ERROR:
            default:
            throw new JSONException("syntax error, pos " + lexer.getBufferPosition()); //	 return null;
        }
    }
        ...
        try {
            if (isEnabled(Feature.AutoCloseSource)) {
                if (lexer.token() != JSONToken.EOF) {
                   throw new JSONException("not close json text, token : " + JSONToken.name(lexer.token()));// return;
                }
            }
        } finally {
            lexer.close();
