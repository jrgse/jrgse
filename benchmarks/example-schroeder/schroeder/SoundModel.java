...
//		System.out.println("---numSampleFrames" + numSampleFrames);
//		System.out.println("---sampleSize" + sampleSize);
//		System.out.println("---sampleRate" + sampleRate);		
//		System.out.println("---" + data_.length);
		if ((data_.length - O2_DATA) >= data.length) {
			System.arraycopy(
					data,
					0,
					data_,
...
//		Debug.printCurrentPC();
		...
		int numBytes = 0;
	  	if (sampleSize % 8 == 0)
	  		numBytes = sampleSize / 8;
	  	else
	  		numBytes = (sampleSize / 8) + 1;

	  	//commented for removing real symbolic experssions
...
//		if( numBytes == 3 ) numBytes = 4;
	...
	private void readFormat() {
		...
		//bytesPerSample_ = (int) Math.ceil( sampleSize_ / 8.0 );
	  	if (sampleSize_ % 8 == 0)
	  		bytesPerSample_ = sampleSize_ / 8;
	  	else
	  		bytesPerSample_ = (sampleSize_ / 8) + 1;
