 ...
 * @see StreamedSource
 */
// TODO: The constructors should be replaced with factory methods for three reasons:
// 1. Provide a non-static configuration mechanism
// 2. Make code more readable by eliminating the requirement to have super constructor as first line in every constructor
// 3. At present can't guarantee that input streams created by the library (e.g. in Source(File) constructor) are closed if an exception occurs.
public final class Source extends Segment implements Iterable<Segment> {
	...
	/**
	 * Constructs a new <code>Source</code> object by loading the content from the specified <code>File</code>.
	 * <p>
	 * The algorithm for detecting the character {@linkplain #getEncoding() encoding} of the source document from the raw bytes
	 * of the specified file is the same as that for the {@link #Source(URLConnection)} constructor,
	 * except that the first step is not possible as there is no
	 * <a target="_blank" href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.17">Content-Type</a> header to check.
	 *
	 * @param file  the <code>java.io.File</code> from which to load the source text.
	 * @throws java.io.IOException if an I/O error occurs.
	 * @see #getEncoding()
	 */
	 ...
	 *   specifying a supported <code>charset</code> parameter, then use the encoding specified in the value of the <code>charset</code> parameter. &diams;
	 *   <br>If the <code>charset</code> parameter is illegally enclosed in double quotes, a {@linkplain Logger#warn(String) warning} is logged and the charset specified inside the quotes is tried.
	 *   <br>If the specified <code>charset</code> is not supported on the host platform, a {@linkplain Logger#warn(String) warning} is logged and the detection algorithm continues.
	 ...
	 * and the following message is logged at {@link Logger#info(String) INFO} level:
	 * "Full sequential parse clearing all tags from cache. Consider calling Source.fullSequentialParse() manually immediately after construction of Source."
	 * <p>
	...
	public Tag[] fullSequentialParse() {
		...
		try {
			useAllTypesCache=false;
			useSpecialTypesCache=false;
			return Tag.parseAll(this,assumeNoNestedTags); // generate constraints
		} finally {
...
////		char a = sourceText.charAt(index);
////		char c = Debug.makeConcolicChar("sym_char_" + index, ""+(int)a);
////	    System.out.println("***********************" + index+"--"+ a + ":"+(int)a);
//	    return c;
	}
