...
public class XMLElement
{
        ...
        throws IOException, XMLParseException
    {
        ...
        for (;;) {
          char ch = this.scanWhitespace(); // that char following a white space. 
            ...
            if ((ch == '!') || (ch == '?')) {
               ...
               this.unreadChar(ch);
             this.scanElement(this); // generate paths
              return;     
            }
        }
    }
...
//            System.out.println("################################read char: " + ch);
    ...
    /***
 * hacked by yufeng
 * not read from the io stream,
 * return a symbolic int directly
 */
...
//    	int i = Debug.makeSymbolicInteger("xml_char_" + (++count));
    	int i = this.reader.read();
//    	char i = Debug.makeConcolicChar("sym_cnf_char_" + (DumpXML.count++), "" + c);
//    	System.out.println("XMLElement.readChar(): " + DumpXML.count + "----" + i + ":" + (int)i);
    	
        if(DumpXML.count >= DumpXML.maxSymbolNum)
        	return (char) -1;
...
//    	Verify.ignoreIf(i < 0);
        if (i < 0) {
            ...
            this.parserLineNr += 1; // read a \n
            return '\n';
        } else {
        	char c = Debug.makeConcolicChar("sym_cnf_char_" + (DumpXML.count++), "" + i);
        	System.out.println("XMLElement.readChar(): " + DumpXML.count + "----" + c + ":" + (int)c);
        	return c;
        }
    }
}
...
//    } else {
//
//    	int i = Debug.makeSymbolicInteger("xml_char_" + (++DumpXML.count));
////        int i = this.reader.read();
//    	Verify.ignoreIf(i < 0);
//    	//following used to follow particular PC, for debug
//    	if(DumpXML.count == 1)
//    		Verify.ignoreIf(i != 60);
//    	if(DumpXML.count == 2)
//    		Verify.ignoreIf(i != 10);
//    	if(DumpXML.count ==3)
//    		Verify.ignoreIf(i != 62);
//    	if(DumpXML.count ==4)
//    		Verify.ignoreIf(i != 38);
//    	if(DumpXML.count ==5)
//    		Verify.ignoreIf(i != 59);
//    	
//    	if(DumpXML.count ==6)
//    		Verify.ignoreIf(i == 59 || i == 10);
//    	if(DumpXML.count ==7)
//    		Verify.ignoreIf(i  < 0);
//    	
//        if (i < 0) {
...
//}
     ...
     */
    protected char readChar_original()
        throws IOException
    {
        if (this.charReadTooMuch != '\0') {
            char ch = this.charReadTooMuch;
         ...
         throws IOException
    {
        ...
        this.scanIdentifier(buf); // scan Identifier can generate some constraints!
...
//        Debug.printPC("########################################prepare to out of range index, PC: \n");
        if (key.length() > 0 && key.charAt(0) == '#') { //bug fix by zhenbang
        //if (key.charAt(0) == '#') { //bug fix by zhenbang        	
            try {
                if (key.charAt(1) == 'x') {
                	//modified by zyf, SPF does not support parseInt with two arguments
                	ch = (char) Integer.parseInt(key.substring(2));
//                	ch = (char) Integer.parseInt(key.substring(2), 16);
                } else {
                	ch = (char) Integer.parseInt(key.substring(1));
